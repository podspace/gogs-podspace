...
if [ "$REDIRECT" == "/install" ]; then
  ...
  curl 'http://localhost:3000/install' -H 'Content-Type: application/x-www-form-urlencoded' --data 'db_type=SQLite3&db_host=127.0.0.1%3A3306&db_user=root&db_passwd=&db_name=gogs&ssl_mode=disable&app_name=Gogs%3A+Go+Git+Service&domain=localhost&ssh_port=22&http_port=3000&app_url=http%3A%2F%2Flocalhost%3A3000%2F&smtp_host=&smtp_from=&smtp_email=&smtp_passwd=&enable_captcha=on&admin_name=&admin_passwd=&admin_confirm_passwd=&admin_email=' --data-urlencode run_user=${RUN_USER} --data-urlencode repo_root_path=${REPO_ROOT} --data-urlencode log_root_path=${LOG_ROOT_PATH} --data-urlencode db_path=${DB_PATH}
